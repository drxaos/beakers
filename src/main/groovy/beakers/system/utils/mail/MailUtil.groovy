...
class MailUtil {
    ...
    /**
     * Unwrap wiser message
     * @param msg WiserMessage
     ...
     * @return unwrapped message
     */
    public static MailMessageContent decodeMessage(MimeMessage mm) {
        def content = getContentFromMessage(mm)
        content.headers = getHeaders(mm)
        content.from = content.headers.get("From")?.find { true }
        content.to = content.headers.get("To")?.find { true }
        content.subject = content.headers.get("Subject")?.find { true }
        def os = new ByteArrayOutputStream()
        mm.writeTo(os)
        content.messageData = os.toByteArray()
        return content
    }
}
